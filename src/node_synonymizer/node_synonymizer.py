...
# Plain-dict replacements for ARAX's openapi_server TRAPI models. Field
# order and default values match openapi_server.models.{node,attribute,
# knowledge_graph}.to_dict() so downstream consumers see an identical
# shape to the original implementation.
def _attribute_dict(
...
class NodeSynonymizer:  # pylint: disable=too-many-instance-attributes
    ...
    def _get_cluster_graph(
        ...
        """Build a TRAPI KnowledgeGraph-shaped dict for a cluster."""
        ...
        return {"nodes": trapi_nodes, "edges": {}}

    def _convert_to_trapi_node(
            self, normalizer_node: dict) -> dict:
        """Convert a normalizer node dict to a TRAPI Node-shaped dict."""
        node = {
        ...
        node["attributes"].append(_attribute_dict(
